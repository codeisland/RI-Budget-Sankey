...
(function () {
  ...
  //---------------- labels --------------------------------------------------------
    ...
    function node_over( its, data, other_nodes, filtered_Links, not_filtered_links ) {
        ...
        not_filtered_links.transition().delay( 150 ).duration( 1500 ).style( pS.link_not_from_node );
        ...
        filtered_Links.transition().delay( 150 ).duration( 1500 ).style( pS.link_from_node );

        other_nodes = pick_all_rect.filter( function ( d ) { // loops through and returns true when not compValue
            return d !== data; 
        } );

        other_nodes.transition().delay( 400 ).duration( 1000 ).style( rS.node_not_hovered );
    ...
    function node_out( its ) {
        ...
        allLinks.transition().delay( 150 ).duration( 1500 ).style( pS.link_init_style );
